...
int gpstime2tm0(unsigned gps_week, double gps_tow, struct gps_tm *res)
{
    ...
    gps_tow = floor(gps_tow * 1e7 + 0.5)/1e7;
    time2epoch(gpst2time(gps_week, gps_tow), ep);

    res->year = (unsigned)ep[0];
    res->month = (unsigned)ep[1];
    res->day = (unsigned)ep[2];
    res->hour = (unsigned)ep[3];
    res->min = (unsigned)ep[4];
    res->sec = ep[5];
